...
//This is minimized version of type 'Complex2'. All operations is inline
static double temp;
inline void operator+=(ShortComplex &x, const Complex2 &y) { x.re += (double)y.re; x.im += (double)y.im; }
inline void operator-=(ShortComplex &x, const Complex2 &y) { x.re -= (double)y.re; x.im -= (double)y.im; }
inline void operator*=(Complex2 &x, const Complex2 &y) { temp = x.re; x.re = temp * y.re - x.im * y.im; x.im = temp * y.im + x.im * y.re; }
inline void operator*=(Complex2 &x, const ShortComplex &y) { temp = x.re; x.re = temp * y.re - x.im * y.im; x.im = temp * y.im + x.im * y.re; }
inline void operator/=(ShortComplex &x, double div) { x.re /= div; x.im /= div; }
inline void operator/=(Complex2 &x, double div) { x.re /= div; x.im /= div; }
...
//exp(-2*pi*j/2^n) = Complex2( cos(2*pi/2^n), -sin(2*pi/2^n) )
static Complex2 W2n[32] = {
...
inline void complex_mul(ShortComplex *z, const ShortComplex *z1, const Complex2 *z2)
{
	z->re = (float)(z1->re * z2->re - z1->im * z2->im);
	z->im = (float)(z1->re * z2->im + z1->im * z2->re);
}
...
static void fft_step(ShortComplex *x, unsigned int T, bool complement, const ShortComplex *Wstore)
{
	...
	for (N = 2, Skew = Nmax, Step = 1; N <= Nmax; N += N, Skew >>= 1, Step++)
	{
		N2 = N + N;
		Nd2 = (N >> 1);
		for (Warray = (float*)Wstore, k = 0; k < Nd2; k++, Warray += Skew)
		{
			Wre = *Warray;
			Wim = cmul * Warray[1];
			for (float *x1re = arr + (k + k); x1re < arrEnd; x1re += N2)
			{
				float *x1im = x1re + 1;
				float *x2re = x1re + N;
				float *x2im = x2re + 1;
...
void universal_fft(ShortComplex *x, int N, bool complement)
{
	...
	{
		arg = -piN * N22*N22;
		w[n].re = (float)cos(arg);
		w[n].im = (float)sin(arg);
	}
	//FFT1
